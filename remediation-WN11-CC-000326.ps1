<#
.SYNOPSIS
    Enables PowerShell Script Block Logging so executed script blocks are recorded to the event log.
...
Write-Host "WN11-CC-000326 applied: $Name = $Value"

# Verify
$Current = (Get-ItemProperty -Path $RegPath -Name $Name -ErrorAction SilentlyContinue).$Name
if ($Current -eq $Value) {
    Write-Host "VERIFIED - WN11-CC-000326 is compliant (current value: $Current)" -ForegroundColor Green
} else {
    Write-Host "FAILED - WN11-CC-000326 did not apply correctly (current value: $Current)" -ForegroundColor Red
}

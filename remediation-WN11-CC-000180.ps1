<#
.SYNOPSIS
    Disables AutoPlay for non-volume devices such as MTP cameras and media players.
...
.USAGE
    ...
    Example syntax:
    PS C:\> .\remediation-WN11-CC-000180.ps1
#>
...
Write-Host "WN11-CC-000180 applied: $Name = $Value"

# Verify
$Current = (Get-ItemProperty -Path $RegPath -Name $Name -ErrorAction SilentlyContinue).$Name
if ($Current -eq $Value) {
    Write-Host "VERIFIED - WN11-CC-000180 is compliant (current value: $Current)" -ForegroundColor Green
} else {
    Write-Host "FAILED - WN11-CC-000180 did not apply correctly (current value: $Current)" -ForegroundColor Red
}

<#
.SYNOPSIS
    Sets the maximum size of the Windows System event log to at least 32768 KB (32 MB).
...
.USAGE
    ...
    Example syntax:
    PS C:\> .\remediation-WN11-AU-000510.ps1
#>
...
Write-Host "WN11-AU-000510 applied: $Name = $Value"

# Verify
$Current = (Get-ItemProperty -Path $RegPath -Name $Name -ErrorAction SilentlyContinue).$Name
if ($Current -eq $Value) {
    Write-Host "VERIFIED - WN11-AU-000510 is compliant (current value: $Current)" -ForegroundColor Green
} else {
    Write-Host "FAILED - WN11-AU-000510 did not apply correctly (current value: $Current)" -ForegroundColor Red
}

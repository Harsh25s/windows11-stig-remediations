<#
.SYNOPSIS
    Prevents standard users from changing Windows Installer installation options.
...
.USAGE
    ...
    Example syntax:
    PS C:\> .\remediation-WN11-CC-000310.ps1
#>
...
Write-Host "WN11-CC-000310 applied: $Name = $Value"

# Verify
$Current = (Get-ItemProperty -Path $RegPath -Name $Name -ErrorAction SilentlyContinue).$Name
if ($Current -eq $Value) {
    Write-Host "VERIFIED - WN11-CC-000310 is compliant (current value: $Current)" -ForegroundColor Green
} else {
    Write-Host "FAILED - WN11-CC-000310 did not apply correctly (current value: $Current)" -ForegroundColor Red
}
